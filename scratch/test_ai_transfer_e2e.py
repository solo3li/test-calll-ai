...
def test_ai_transfer():
    ...
    print(f"[+] Found tenant user: {user.username} (ID: {user.id})")

    # 1. Test Bootstrap API includes call_queues
    ...
    print(f"[+] Bootstrap returned {len(queues)} queues for tenant {user.username}:")
    for q in queues:
