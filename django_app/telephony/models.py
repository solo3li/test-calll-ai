...
class InboundPBXTrunk(models.Model):
    ...
    def __str__(self):
        status = " [نشط]" if self.is_active else " [معطل]"
        return f"{self.name} ({self.get_auth_mode_display()}){status}"

    def generate_issabel_config(self, host_domain="app.localhost", sip_port=5060):
        trunk_name = f"livekit-ai-{self.id}"
        lines = [
            f"; ===========================================================",
            f"; اعدادات ربط السنترال الخاص بـ Issabel / FreePBX / Asterisk",
            f"; انسخ هذا النص وضعه في: PBX -> Trunks -> Add SIP Trunk",
