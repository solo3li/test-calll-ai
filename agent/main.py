...
def build_dynamic_system_instruction(profile: dict, memory_card: str = "", queue_context: dict = None, outbound_context: dict = None, call_queues: list = None) -> str:
    ...
    # 2. Dialect and Language rules & explicit apology text
    DIALECT_RULES_MAP = {
        "saudi": (
...
async def run_agent_session(room_name: str, user_id: int = None, caller_phone: str = "web_dashboard", profile_data: dict = None, queue_context: dict = None, outbound_context: dict = None):
    ...
    logger.info(f"Using Google voice '{chosen_voice}', language '{active_profile.get('language', 'arabic')}', dialect '{active_profile.get('dialect')}', gender '{active_profile.get('gender')}' for user {user_id}")
    ...
    if call_queues:
        ...
        transfer_decl = {
            ...
            "description": f"تحويل المكالمة الجارية إلى أحد طوابير الموظفين البشريين التابعة للمؤسسة. الأقسام واختصاصاتها: [{q_descriptions}]. استدعِ هذه الأداة فوراً عند طلب العميل التحدث مع موظف بشري أو الاستفسار عن قسم معين يطابق اختصاصات الطابور.",
            "parameters": {
                "type": "OBJECT",
                "properties": {
                    "queue_code": {
